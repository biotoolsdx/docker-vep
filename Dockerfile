...
RUN git clone -b $BRANCH --depth 1 git://github.com/Ensembl/ensembl.git && \
...
#    rm -rf MMSplice && \
    ...
    git clone -b $BRANCH --depth 1 https://github.com/Ensembl/ensembl-vep.git && chmod u+x ensembl-vep/*.pl && \
    # Clone ensembl-variation git repository and compile C code
    git clone -b $BRANCH --depth 1 https://github.com/Ensembl/ensembl-variation.git && \
    mkdir var_c_code && \
...
#    add-apt-repository -y ppa:jonathonf/python-3.6  && apt-get update && \
    ...
    build-essential \
    cpanminus \
    curl libcurl4-openssl-dev  libbz2-dev liblzma-dev libz-dev \
    libmysqlclient-dev \
    libpng-dev \
    libssl-dev \
    locales \
...
#    python3-pip \
    ...
    apt-get clean && \
    rm -rf /var/lib/apt/lists/* /usr/bin/python3 /usr/bin/python3m && ln -s /usr/bin/python3.6 /usr/bin/python3 && \
    ln -s /usr/bin/python3.6 /usr/bin/python && ln -s /usr/bin/python3.6m /usr/bin/python3m

# Setup VEP environment
ENV OPT /opt/vep
ENV OPT_SRC $OPT/src
ENV PERL5LIB_TMP $PERL5LIB:$OPT_SRC/ensembl-vep:$OPT_SRC/ensembl-vep/modules:$OPT_SRC/Plugins
...
RUN echo >> $OPT/.profile && \
    ...
    ./INSTALL.pl -a ap -l -g ExAC && rm -rf t travisci .travis.yml

#RUN pip3 install numpy==1.16.4 cython cyvcf2 pyBigWig && \
#    pip3 install mmsplice==0.2.7
